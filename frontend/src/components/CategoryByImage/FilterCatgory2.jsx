...
const FilterCatgory2 = () => {
  ...
  const level1Categories = data.filter((item) => item.level === 2);
     ...
     const visibleCategories = [...(level1Categories || [])].sort((a, b) =>
          a?.category?.SubCategoryName?.localeCompare(b?.category?.SubCategoryName)
        );
          ...
          >
            <Image
              src={`${serverUrl}/public/image/${category.levelImage}`}
                           alt={category?.category?.SubCategoryName || "Image"}

              width={400}
              height={250}
              className="w-full h-[280px] md:h-[380px] object-fill bg-top transition-transform duration-300 group-hover:scale-105"
            />
            <div className="absolute bottom-0 w-full flex items-center justify-center pb-2">
              <button className="yellow-bg cursor-pointer hover:bg-purple-800 text-white font-medium px-4 py-2 rounded-full inline-flex items-center gap-2 transition ">
