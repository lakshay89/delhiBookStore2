...
import image1 from "../../app/Images/DBS/1.jpg"; // fallback image
...
const apiCartCount = useSelector((state) => state.apiCart.items?.length);
  ...
  return (
    <>
        ...
        <div className="md:hidden fixed top-0 left-0 w-full z-50 bg-white p-3 shadow-md flex flex-col gap-2 transition-all duration-300">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-[#da7921] text-black rounded-full focus:outline-none"
            />
            <Search className="blue-color" h={16} w={16} />
          </div>
      ...
      {/* Bottom Nav Bar */}
      <div className="fixed bottom-0 left-0 z-40 w-full h-16 bg-[#da7921] rounded-t-lg md:hidden">
        <div className="grid h-full grid-cols-5 mx-auto">
          {navItems.map((item) => {
            ...
            if (isSearch) {
              return (
                <button
                  ...
                  className="relative -top-4 cursor-pointer w-14 h-14 bg-white text-purple-900 rounded-full flex items-center justify-center shadow-lg border-2 border-[#ec8728] mx-auto"
                >
                  {item.icon}
                </button>
              );
            ...
            return (
              ...
              >
                <div className={`${isActive ? "text-[#ffcf9c]" : "text-white"}`}>
                  {item.icon}
                  {item.count > 0 && (
                ...
                <span
                  className={`text-xs mt-1 ${
                    isActive ? "text-[#ffcf9c]" : "text-white"
                  }`}
                >
                  {item.label}
                </span>
